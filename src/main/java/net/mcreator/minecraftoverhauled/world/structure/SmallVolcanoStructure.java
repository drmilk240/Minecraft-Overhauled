...
public class SmallVolcanoStructure extends MinecraftOverhauledModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public SmallVolcanoStructure(MinecraftOverhauledModElements instance) {
		super(instance, 243);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 20000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("minecraft_overhauled", "smallvolcano"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("small_volcano"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("minecraft_overhauled:small_volcano"), configuredFeature);
		}
	}
	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("minecraft_overhauled:volcanic_geysers").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
